********************************************************************************
****   Project: COVID-19 State Paper (Latest Update: 07/05/2020)
...
merge 1:1 statefips year month day using "$dataoutput\lowincearning_small_state_daily.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\lowincemp_all_state_daily.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\lowincemp_small_state_daily.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\UIclaims_state_weekly.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\Womply_merchants_state_daily.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\Womply_revenues_state_daily.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\Zearn_state_weekly.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\COVIDcases_state_daily.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\COVIDdeths_state_daily.dta", nogen
merge 1:1 statefips year month day using "$dataoutput\COVIDtests_state_daily.dta", nogen

save "$dataoutput\all_combined_state.dta", replace
